...
- **Update (2026-06-26, execution-time): the `EXPRESSION_FILTER` walker was dropped (Tasks 5–6 skipped).** Probing 12 predicate shapes against a registered arrow view showed DuckDB v1.5.4 **never emits `EXPRESSION_FILTER` for arrow scans** — it constant-folds expressions, wraps OR/IN in `OPTIONAL_FILTER`, and routes the rest to `CONSTANT_COMPARISON` / `CONJUNCTION_*` / `STRUCT_EXTRACT` / `IN_FILTER` (all handled by Tasks 1–4) or keeps them above the scan. The walker would be dead, untestable code on v1.5.4. Coverage parity for v1.5.4 is therefore achieved by the structured path (Tasks 1–4); the existing `UNHANDLED → error_stream` remains the correctness-safe fallback for the (non-occurring) `EXPRESSION_FILTER`. The expression-tree walker is the path duckdb-python needs on DuckDB **main** (post-LEGACY migration); revisit when/if the bundle is bumped to such a release.
