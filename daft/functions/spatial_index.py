...
def _cell_for_point(h3lib: Any, is_v4: bool, lat: float, lon: float, res: int) -> str:
    if is_v4:
        return h3lib.latlng_to_cell(lat, lon, res)
    return h3lib.geo_to_h3(lat, lon, res)


def _disk(h3lib: Any, is_v4: bool, cell: str, k: int) -> set[str]:
    if is_v4:
        ...
        if estimated <= max_polyfill:
            ...
            try:
                if is_v4:
                    ...
                    geojson_dict = {"type": "Polygon", "coordinates": [[[lon, lat] for lat, lon in outer_latlng]]}
                    interior = set(h3lib.polyfill_geojson(geojson_dict, resolution))
...
def _file_mbr_from_wkbs(wkbs: list[Any]) -> tuple[float, float, float, float] | None:
    ...
    raw_mbrs = [_wkb_mbr(w) for w in valid]
    valid_mbrs_list: list[tuple[float, float, float, float]] = [m for m in raw_mbrs if m is not None]
    if not valid_mbrs_list:
        return None
    return (
        min(m[0] for m in valid_mbrs_list),
        min(m[1] for m in valid_mbrs_list),
        max(m[2] for m in valid_mbrs_list),
        max(m[3] for m in valid_mbrs_list),
    )
...
def _build_h3_index(
    ...
    def _scan_file(fpath: str) -> tuple[tuple[float, float, float, float] | None, bool]:
        ...
        if _BBOX_COLS[0] in schema_names and all(c in schema_names for c in _BBOX_COLS):
            ...
            if not xs0:
                return None, False
            mbr_val: tuple[float, float, float, float] | None = (min(xs0), min(ys0), max(xs1), max(ys1))
            # Polygons always have bbox cols; treat as non-point
            return mbr_val, False
    ...
    def _index_one_file(fpath: str) -> tuple[str, list[str]]:
        ...
        if is_pts:
            ...
            x0, y0, x1, y1 = mbr
            cells = _h3_cells_for_file_mbr(h3lib, is_v4, x0, y0, x1, y1, effective_resolution)
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        idx_futs = {pool.submit(_index_one_file, fp): fp for fp in parquet_files}
        for idx_fut in as_completed(idx_futs):
            basename, cell_list = idx_fut.result()
            if cell_list:
...
class SpatialIndex:
    ...
    def _load_full(self) -> None:
        ...
        for cell, fname in zip(tbl["h3_cell"].to_pylist(), tbl["filename"].to_pylist()):
            if fname is None or cell is None:
                continue
            d.setdefault(str(fname), []).append(str(cell))
    ...
    def _filter_h3(self, full_paths: list[str], query_wkb: bytes) -> list[str]:
        ...
        matched_basenames: set[str] = {str(v) for v in tbl["filename"].to_pylist() if v is not None}
    ...
    def _all_indexed_files(self) -> set[str]:
        ...
        if not hasattr(self, "_indexed_files_cache"):
            if self.index_path is None:
                ...
                self._indexed_files_cache = {str(v) for v in tbl["filename"].unique().to_pylist() if v is not None}
...
def spatial_join(
    ...
    *,
    ...
    def _to_df_and_dir(arg: Any, default_dir: str | None) -> tuple[Any, str | None]:
    ...
    def _geohash_prefix_pruning(probe_dir: str, build_df: Any, build_geom_col: str) -> list[str] | None:
    ...
    def _prune_dir(probe_dir: str, probe_geom: str, build_df: Any, build_geom: str) -> list[str] | None:
